...
Store maintain a current offset number. Whenever a message is stored, it is assigned the current offset number and after message is stored successfully current offset number is incremented by 1. While storing message it is first stored in store file and once store return the offset and position of the message an entry is made to index file.
